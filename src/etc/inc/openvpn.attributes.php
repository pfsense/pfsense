...
function parse_cisco_acl_rule($rule, $devname, $dir) {
    ...
    if (trim($rule[$index]) == "host") {
        ...
        if(!is_ipaddr($network)) {
            syslog(LOG_WARNING, "Error parsing rule {$rule_orig}: Invalid network.");
            return;
        }
        ...
        try {
            ...
            syslog(LOG_WARNING, "Error parsing rule {$rule_orig}: Invalid netmask.");
            return;
        }
    ...
    if (in_array(trim($rule[$index]), array("lt", "gt", "dq", "neq"))) {
        ...
        if (is_numeric($port)) {
            ...
            syslog(LOG_WARNING, "Error parsing rule {$rule_orig}: Invalid port.");
            return;
        }
        $index++;
    } else if (trim($rule[$index]) == "range") {
        $port = array($rule[++$index], $rule[++$index]);
        if (is_numeric($port[0]) && is_numeric($port[1])) {
            $port[0]--;
            $port[1]++;
