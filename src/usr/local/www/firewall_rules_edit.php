  ...
  if ($type == 'src') {
    $section->addInput(new Form_Button(
      ...
      'The %1$sSource Port Range%2$s for a connection is typically random ' .
        ...
        'In most cases this setting must remain at its default value, %1$sany%2$s.',
      '<b>',
      '</b>'
    );
  }
...
$section->addInput(new Form_Select(
  ...
  'Select which type of state tracking mechanism to use.  If in doubt, use keep state.%1$s',
  '<br /><span></span>'
);
...
$section->add($group)->setHelp(
  'Choose the Out queue/Virtual interface only if ' .
...
$section->add($group)->setHelp(
  'Choose the Acknowledge Queue only if there is a ' .
    'selected Queue.'
);
...
<script type="text/javascript">
  ...
  events.push(function() {
    ...
    function show_advopts(ispageload) {
      ...
      if (ispageload) {
        showadvopts = <?php if (is_aoadv_used($pconfig)) {
                        echo 'true';
                      } else {
                        echo 'false';
                      } ?>;
