...
if ($_POST) {
	if ($_POST['apply']) {
		...
		if (!empty($_POST['dns64_prefix']) && !validate_nat64_prefix($_POST['dns64_prefix'])) {
			$input_errors[] = gettext("The DNS64 prefix is invalid.");
		}
		...
		if (!$input_errors) {
			...
			if (isset($_POST['dns64_enable'])) {
				config_set_path('unbound/dns64/enable', true);
			} else {
				config_del_path('unbound/dns64/enable');
			}
			if (config_path_enabled('system', 'allow_nat64_prefix_override') && !empty($_POST['dns64_prefix'])) {
