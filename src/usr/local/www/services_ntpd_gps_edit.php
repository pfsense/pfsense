...
##|+PRIV
##|*IDENT=page-services-ntpd-gps-edit
##|*NAME=Services: NTP Serial GPS Edit
##|*DESCR=Allow access to the 'Services: NTP Serial GPS Edit' page.
##|*MATCH=services_ntpd_gps_edit.php*
##|-PRIV
...
if (isset($id) && $a_gps[$id]) {
	$pconfig = $a_gps[$id];
} else {
	$pconfig['type'] = 'Default';
}
...
if ($_POST) {
	...
	if (!empty($_POST['gpsport']) && file_exists('/dev/'.$_POST['gpsport'])) {
		$gps['port'] = $_POST['gpsport'];
	}

	if (!empty($_POST['gpstype'])) {
		$gps['type'] = $_POST['gpstype'];
	}

	if (!empty($_POST['gpsspeed'])) {
		$gps['speed'] = $_POST['gpsspeed'];
	} else {
		$gps['speed'] = 0;
	}
	...
	if (!empty($_POST['gpsnmea']) && ($_POST['gpsnmea'][0] === "0")) {
		...
		$gps['nmea'] = strval(array_sum($_POST['gpsnmea']));
	}
	
	if (!empty($_POST['processpgrmf'])) {
		$gps['processpgrmf'] = $_POST['processpgrmf'];
	}

	if (!empty($_POST['gpsfudge1'])) {
		$gps['fudge1'] = $_POST['gpsfudge1'];
	}

	if (!empty($_POST['gpsfudge2'])) {
		$gps['fudge2'] = $_POST['gpsfudge2'];
	}

	if (!empty($_POST['gpsstratum']) && ($_POST['gpsstratum']) < 16) {
		$gps['stratum'] = $_POST['gpsstratum'];
	}

	if (!empty($_POST['gpsprefer'])) {
		$gps['prefer'] = $_POST['gpsprefer'];
	}

	if (!empty($_POST['gpsnoselect'])) {
		$gps['noselect'] = $_POST['gpsnoselect'];
	}

	if (!empty($_POST['gpsflag1'])) {
		$gps['flag1'] = $_POST['gpsflag1'];
	}

	if (!empty($_POST['gpsflag2'])) {
		$gps['flag2'] = $_POST['gpsflag2'];
	}

	if (!empty($_POST['gpsflag3'])) {
		$gps['flag3'] = $_POST['gpsflag3'];
	}

	if (!empty($_POST['gpsflag4'])) {
		$gps['flag4'] = $_POST['gpsflag4'];
	}

	if (!empty($_POST['gpssubsec'])) {
		$gps['subsec'] = $_POST['gpssubsec'];
	}

	if (!empty($_POST['gpsrefid'])) {
		$gps['refid'] = $_POST['gpsrefid'];
	}

	if (!empty($_POST['extstatus'])) {
		$gps['extstatus'] = $_POST['extstatus'];
	}

	if (!empty($_POST['autocorrect_initcmd'])) {
		$gps['autocorrect_initcmd'] = $_POST['autocorrect_initcmd'];
	}

	if (!empty($_POST['gpsinitcmd'])) {
		$initcmd = $_POST['gpsinitcmd'];
		if ($gps['autocorrect_initcmd']) {
			$initcmd = autocorrect_initcmd($initcmd);
		}
		$gps['initcmd'] = base64_encode($initcmd);
		parse_initcmd($gps['nmeaset'], $initcmd);
	}
...
$pgtitle = array(gettext("Services"), gettext("NTP"), gettext("Serial GPS"), gettext("Edit"));
...
events.push(function() {
	...
	function show_advgps(ispageload) {
		...
		if (ispageload) {
			...
			if (('<?=$pconfig['initcmd'] ?>' == get_base64_gps_string($('#gpstype').val())) || ('<?=$pconfig['initcmd']?>' == '')) {
				showadvgps = false;
			} else {
				showadvgps = true;
			}
