...
$section->addInput(new Form_Checkbox(
	...
	$pconfig['forwarding']
))->setHelp(sprintf(gettext('If this option is set, DNS queries will be forwarded to the upstream DNS servers defined under'.
...
$section->addInput(new Form_Checkbox(
	...
	$pconfig['regdhcp']
))->setHelp(sprintf(gettext('If this option is set, then machines that specify their hostname when requesting a DHCP lease will be registered'.
...
))->setHelp(sprintf(gettext('If this option is set, then DHCP static mappings will be registered in the DNS Resolver, so that their name can be resolved. '.
					'The domain in %1$sSystem &gt; General Setup%2$s should also be set to the proper value.'), '<a href="system.php">', '</a>'));
