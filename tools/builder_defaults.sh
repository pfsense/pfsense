...
export FREEBSD_BRANCH=${FREEBSD_BRANCH:-"factory-RELENG_2_4_4"}
...
if [ -z "${MODULES_OVERRIDE}" ]; then
	...
	export MODULES_OVERRIDE_amd64="${MODULES_OVERRIDE_armv6} aesni amdtemp coretemp cpuctl drm2 ena ichwd ipmi i2c/controllers/ismt ix ixv linuxkpi ndis nmdm sfxge udf vmm"
...
export VIRT_UFS=${VIRT_UFS:-${OVFUFS}}
export VIRT_DISK_CAPACITY_IN_GB=${VIRT_DISK_CAPACITY_IN_GB:-${VMDK_DISK_CAPACITY_IN_GB}}
export VIRT_SWAP_PART_SIZE_IN_GB=${VIRT_SWAP_PART_SIZE_IN_GB:-${VMDK_SWAP_PART_SIZE_IN_GB}}
export VIRT_TMP=${VIRT_TMP:-${OVA_TMP}}
...
export PKG_RSYNC_DESTDIR=${PKG_RSYNC_DESTDIR:-"/storage/files/release-staging/factory/packages"}

# Final packages server
if [ -n "${_IS_RELEASE}" -o -n "${_IS_RC}" ]; then
	export PKG_FINAL_RSYNC_HOSTNAME=${PKG_FINAL_RSYNC_HOSTNAME:-"nfs1.nyi.netgate.com"}
	export PKG_FINAL_RSYNC_DESTDIR=${PKG_FINAL_RSYNC_DESTDIR:-"/storage/files/firmware/pkg"}
else
	export PKG_FINAL_RSYNC_HOSTNAME=${PKG_FINAL_RSYNC_HOSTNAME:-"nfs1.nyi.netgate.com"}
	export PKG_FINAL_RSYNC_DESTDIR=${PKG_FINAL_RSYNC_DESTDIR:-"/storage/files/firmware/beta/packages"}
fi
...
if [ "${TARGET_ARCH}" == "aarch64" ]; then
	export PKG_REPO_SERVER_DEVEL=${PKG_REPO_SERVER_DEVEL:-"pkg+https://repo.netgate.com/beta/packages"}
	export PKG_REPO_SERVER_RELEASE=${PKG_REPO_SERVER_RELEASE:-"pkg+https://repo.netgate.com/pkg"}
else
...
export PLCCBPATH=${PLCCBPATH:-"${IMAGES_FINAL_DIR}/installer/${PRODUCT_NAME}${PRODUCT_NAME_SUFFIX}-memstick-XG-7100-${PRODUCT_VERSION}${PRODUCT_REVISION:+-p}${PRODUCT_REVISION}-${TARGET}${TIMESTAMP_SUFFIX}.img"}
...
export VIRTPATH_TMPL=${VIRTPATH_TMPL:-"${IMAGES_FINAL_DIR}/virtualization/${PRODUCT_NAME}${PRODUCT_NAME_SUFFIX}-%%TYPE%%-${PRODUCT_VERSION}${PRODUCT_REVISION:+-p}${PRODUCT_REVISION}-${TARGET}${TIMESTAMP_SUFFIX}.%%EXT%%"}
export EXTRA_IMAGES=""
...
if [ -n "${_IS_RELEASE}" -o -n "${SKIP_FINAL_RSYNC}" ]; then
	...
	export RSYNCPATH=${RSYNCPATH:-"/storage/files/release-staging/factory/images"}
else
	export RSYNCIP=${RSYNCIP:-"nfs1.nyi.netgate.com"}
	export RSYNCUSER=${RSYNCUSER:-"wwwsync"}
...
if [ "${PRODUCT_NAME}" = "pfSense" ]; then
	...
	export OVF_INFO=${OVF_INFO:-"The Netgate pfSense Certified firewall and VPN appliance runs pfSense, a customized distribution of FreeBSD tailored for use as a firewall and router. In addition to being a powerful, flexible firewalling and routing platform, it includes a long list of related features and a package system allowing further expandability without adding bloat and potential security vulnerabilities to the base distribution."}
else
